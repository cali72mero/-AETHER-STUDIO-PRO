...
[![Status: Beta](https://img.shields.io/badge/Status-Beta%20v2.7.2.4-yellow.svg)](#-community-feedback--bug-reports)
...
> [!NOTE]
> This software is independently developed and maintained by a **single solo developer**. The software is currently in active **Beta phase (v2.7.2.4 Beta)**.
> 
...
>   All versions **from v2.7 onwards** (v2.7.0, v2.7.1, v2.7.2, v2.7.2.1, v2.7.2.2, v2.7.2.3, v2.7.2.4) are active **Beta versions** featuring cutting-edge experimental features.  
...
> 
> ### ⚠️ Face Swap Feature Status (Under Active Maintenance / Known Bugs)
> **Please note:** The **Live Face Swap & Smartphone Remote Camera** feature is currently **not fully usable or only works with errors/bugs** on certain devices and browsers.  
> **This bug is actively being addressed and will be resolved in an upcoming update!**  
> All core Fooocus features (high-fidelity image generation, inpainting, outpainting, Vision AI Studio, prompt expansion, and custom sampling steps) remain 100% stable and functional.

---

## 🔄 Version Evolution: Fooocus ➔ v2.5 ➔ v2.6 ➔ v2.6.3 ➔ v2.7.0 ➔ v2.7.1 ➔ v2.7.2 ➔ v2.7.2.1 ➔ v2.7.2.2 ➔ v2.7.2.3 ➔ v2.7.2.4

### 🆕 What's New in v2.7.2.4 (Current Patch - iOS Safari WebKit Touch & Inline Video Autoplay Fix)
1. **📱 iOS Safari & Mobile WebKit Inline Video Autoplay**:
   - Added `webkit-playsinline`, `playsinline`, and explicit JS `muted = true` / `defaultMuted = true` to prevent iOS Safari from blocking camera video or hanging.
   - Non-blocking `video.play()` error handling with metadata deferral.
2. **🎛️ In-Page Diagnostics & Error Card**:
   - Replaced browser `alert()` (which is silently suppressed by modern iOS Safari pop-up blockers) with an interactive on-page error card with clear status emojis and actionable instructions.
   - Instant visual button feedback (`⌛ Kamera wird gestartet...`) and dual `touchend` + `click` event listeners for mobile response without 300ms tap delay.
3. **⚡ Dynamic Frame Loop Protection**:
   - In-flight request lock (`isSending`) preventing network congestion during high-FPS smartphone camera streaming.

---

### 🌟 What was added in v2.7.2.3 (iPhone Safari HTTPS Cloud-Tunnel & LAN Listen)
1. **📱 iPhone & Safari Remote Webcam HTTPS Cloud-Tunnel**:
   - Fixed `TypeError: expected str, not NoneType` by supplying a valid 32-byte secret token (`secrets.token_urlsafe(32)`).
   - Real HTTPS cloud URL (`https://....gradio.live`) enabling camera permission on iOS Safari.
2. **🌐 Default LAN Network Binding (`listen="0.0.0.0"`)**:
   - Default `listen="0.0.0.0"` in `args_manager.py` and `--listen` in `run_lowvram.sh`.
...
>   Alle Versionen **ab 2.7** (v2.7.0, v2.7.1, v2.7.2, v2.7.2.1, v2.7.2.2, v2.7.2.3, v2.7.2.4) sind **aktive Beta-Versionen** mit neuesten experimentellen Funktionen (Live Face Swap & Avatar Studio, kabellose Smartphone-Kamera, Precision Vision AI, Custom Performance Steps).  
...
## 🔄 Versions-Evolution: Was ist neu in v2.7.2.4?

1. **📱 iOS Safari & WebKit Inline-Video Autoplay Fix**:
   - `webkit-playsinline`, `playsinline` und explizites `muted = true` ergänzt, damit iOS Safari das Kamerabild nicht mehr blockiert oder beim Abspielen einfriert.
...
### 🌟 Was wurde in v2.7.2.3 & v2.7.2.2 hinzugefügt?
1. **📱 HTTPS Cloud-Tunnel Token Fix**: Sichere HTTPS-Verbindung (`https://....gradio.live`) für iOS Safari Kameraerlaubnis.
2. **🌐 Standard LAN-Bindung (`listen="0.0.0.0"`)**: Fooocus bindet standardmäßig an alle Netzwerkadressen.
