    ...
    return f"""
        <div class="aether-header-bar-inner">
            <div class="aether-brand-title-group">
                ...
                <span class="aether-studio-version">v2.7.2.2 NEXT-GEN</span>
            ...
            <div class="aether-status-group">
                <span class="aether-sys-live-dot">●</span>
                <span class="aether-status-text">SYSTEM BEREIT</span>
            </div>
        </div>
    """
...
def render_vram_info(is_eco: bool = True):
    if is_eco:
        return """
### 🌱 VRAM-Sparmodus (Eco / Low-VRAM) — AKTIV
- **Grafikspeicher-Bedarf:** Nur ca. 3,5–4,5 GB VRAM belegt.
- **Funktionsweise:** Schont deine 6 GB VRAM der RTX 3050, indem ungenutzte Modellschichten automatisch in den 32 GB System-RAM zwischengespeichert werden.
- **Ideal für:** Große SDXL-Checkpoints, gleichzeitiges Verwenden mehrerer LoRAs oder Arbeiten mit anderen grafikintensiven Programmen im Hintergrund.
- **Empfehlung:** Standardmäßig aktiv lassen für höchste Stabilität ohne Abstürze!
"""
    else:
        return """
### 🚀 Normalmodus (Maximale GPU-Power) — AKTIV
- **Grafikspeicher-Bedarf:** Vollständige Nutzung des verfügbaren VRAMs.
- **Funktionsweise:** Lädt das Diffusionsmodell so weit wie möglich direkt auf die Grafikkarte (CUDA-Speicher).
- **Vorteile:**
...
title = 'Aether Diffusion Studio Pro 2.7.2.2 • Autonomous Neural AI'
...
""")
                    ...
                    with gr.Tab(label='🎭 Live Face Swap & Avatar Studio (Beta)', id='faceswap_tab') as faceswap_tab:
                        ...
                        def on_start_tunnel():
                            ...
                            if pub_url:
                                info_md = f"""
                            **📱 Smartphone / Tablet als drahtlose HD-Webcam nutzen (🟢 Öffentlicher HTTPS-Tunnel aktiv):**
                            1. **Funktioniert garantiert auf iPhone (Safari) & Android**, auch über Mobilfunk (4G/5G) oder außerhalb deines WLANs!
                            2. Öffne im Handy-Browser die sichere HTTPS-Adresse:  
                               👉 **[{target_url}]({target_url})**
                            3. Melde dich an mit: Benutzer: `aether` | PIN: **`{creds['pin']}`**
                            4. Tippe auf **„📸 Kamera starten“** – erlaube den Kamerazugriff im Browser!
                            """
                                status_txt = f"🟢 **Öffentlicher HTTPS-Tunnel aktiv!** ({pub_url}) • Kamera für iPhone freigegeben"
                            else:
                                info_md = f"""
                            **📱 Smartphone / Tablet als drahtlose HD-Webcam nutzen:**
                            *(Hinweis: Cloud-Tunnel konnte nicht gestartet werden, lokale WLAN-Adresse wird verwendet)*
                            1. Verbinde dein Handy mit demselben WLAN wie deinen PC.
                            2. Öffne im Handy-Browser: **[{target_url}]({target_url})**
                            3. Melde dich an mit: Benutzer: `aether` | PIN: **`{creds['pin']}`**
                            4. Tippe auf **„📸 Kamera starten“**.
                            """
                                status_txt = "⚠️ Cloud-Tunnel fehlgeschlagen. Lokale WLAN-Adresse aktiv."
                            qr_html = f"""
                            <div style="text-align: center; background: #fff; padding: 10px; border-radius: 14px; width: 150px; margin: 0 auto; box-shadow: 0 4px 15px rgba(0,0,0,0.4);">
                                <img src="https://api.qrserver.com/v1/create-qr-code/?size=130x130&data={target_url}" alt="QR Code" style="width: 130px; height: 130px; display: block; margin: 0 auto;" />
                                <span style="color: #0f172a; font-size: 0.72rem; font-weight: 800; display: block; margin-top: 6px; letter-spacing: 0.04em;">MIT HANDY SCANNEN</span>
        ...
        with gr.Column(scale=1, visible=modules.config.default_advanced_checkbox) as advanced_column:
            with gr.Tab(label='⚙️ Settings & Performance'):
                ...
                with gr.Row(visible=(modules.config.default_performance == flags.Performance.CUSTOM.value), elem_id='custom_perf_row') as custom_perf_row:
                    with gr.Column(scale=3):
                        custom_perf_steps = gr.Slider(
                            label='🎯 Eigene Sampling Steps (Schritte frei einstellen)',
                            minimum=1,
                            maximum=200,
                            step=1,
                            value=30,
                            info='Wähle eine frei wählbare Schrittanzahl für die Bildgenerierung (z. B. 15, 25, 45, 50, 75, 100).'
                        )
                    with gr.Column(scale=1):
                        custom_perf_steps_num = gr.Number(
                            label='🔢 Schritt-Direkteingabe',
                            value=30,
                            precision=0,
                            minimum=1,
                            maximum=200,
                            info='Exakte Schrittzahl'
                        )
...
| **🎯 Custom** | **1–200 Steps** | Frei wählbar | Frei einstellbar | Völlig freie Schrittanzahl für eigene Feinabstimmungen & Experimente |
...
- **Custom (Eigene Schritte):** Wenn du eigene Sampling-Schritte (z. B. 20, 25, 40, 50 oder 80 Steps) direkt festlegen möchtest.
                    """)
...
- **Zielordner:** `{data['target_dir']}`
        ...
        def on_perf_change(perf_val, current_custom_steps):
            is_custom = (perf_val == flags.Performance.CUSTOM.value)
            overwrite_val = int(current_custom_steps) if is_custom else -1
            return gr.update(visible=is_custom), gr.update(value=overwrite_val)

        performance_selection.change(
            on_perf_change,
            inputs=[performance_selection, custom_perf_steps],
            outputs=[custom_perf_row, overwrite_step],
            queue=False,
            show_progress=False
        )

        def on_custom_slider_change(val, perf_val):
            val_int = int(val)
            is_custom = (perf_val == flags.Performance.CUSTOM.value)
            overwrite_val = val_int if is_custom else -1
            return gr.update(value=val_int), gr.update(value=overwrite_val)

        custom_perf_steps.change(
            on_custom_slider_change,
            inputs=[custom_perf_steps, performance_selection],
            outputs=[custom_perf_steps_num, overwrite_step],
            queue=False,
            show_progress=False
        )

        def on_custom_num_change(val, perf_val):
            try:
                val_int = max(1, min(200, int(val)))
            except:
                val_int = 30
            is_custom = (perf_val == flags.Performance.CUSTOM.value)
            overwrite_val = val_int if is_custom else -1
        ...
        custom_perf_steps_num.change(
            on_custom_num_change,
            inputs=[custom_perf_steps_num, performance_selection],
            outputs=[custom_perf_steps, overwrite_step],
            queue=False,
            show_progress=False
        )

        def on_overwrite_step_sync(ov_val):
            if ov_val > 0:
                return gr.update(value=ov_val), gr.update(value=ov_val)
            return gr.update(), gr.update()

        overwrite_step.change(
            on_overwrite_step_sync,
            inputs=[overwrite_step],
            outputs=[custom_perf_steps, custom_perf_steps_num],
            queue=False,
            show_progress=False
        )
...
<body>
    ...
    <!-- iOS / HTTP Info Banner -->
    <div id="ios-http-banner" style="display: none; background: rgba(245, 158, 11, 0.18); border: 1px solid rgba(245, 158, 11, 0.5); border-radius: 14px; padding: 12px; margin-bottom: 14px; max-width: 440px; width: 100%; font-size: 0.85rem; color: #fde68a; line-height: 1.4;">
        ⚠️ <strong>Wichtiger Hinweis:</strong> Auf Smartphones erlauben Safari und Chrome die Kamera nur über <strong>HTTPS</strong>! Bitte nutze den <strong>„🌐 Öffentlichen Cloud-Tunnel“</strong> (beginnt mit <strong>https://</strong>).
    </div>
    ...
    <div id="cam-screen" class="card">
        ...
        <div id="error-box" style="display: none; background: rgba(239, 68, 68, 0.2); border: 1px solid rgba(239, 68, 68, 0.5); border-radius: 12px; padding: 12px; margin-bottom: 12px; color: #fca5a5; font-size: 0.85rem; line-height: 1.4;"></div>

        <div class="video-box">
            <video id="webcam-video" playsinline webkit-playsinline autoplay muted></video>
    ...
    <script>
        ...
        // Detect HTTP on mobile
        if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
            const banner = document.getElementById('ios-http-banner');
        ...
        async function startCamera() {
            ...
            startCamBtn.innerText = '⌛ Kamera wird gestartet...';
            statusBadge.className = 'status-badge';
            statusBadge.innerText = '🟡 Öffne Kamera...';

            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
                    showError(`
                        <strong>⚠️ Kamerazugriff vom Smartphone-Browser gesperrt!</strong><br><br>
                        Du bist über unverschlüsseltes <code>http://</code> verbunden. Auf iPhones (Safari) und Android verbieten alle Browser den Kamerazugriff ohne HTTPS!<br><br>
                        👉 <strong>So geht es sofort:</strong><br>
                        1. Gehe an deinen PC in Aether Studio Pro.<br>
                        2. Klicke auf <strong>„🌐 Öffentlichen Cloud-Tunnel starten (für 4G/5G)“</strong>.<br>
                        3. Scanne den neuen QR-Code (beginnt mit <strong>https://</strong>).<br>
                        Über die sichere HTTPS-Verbindung fragt dich das Smartphone nach der Kameraerlaubnis!
                    `);
                    return;
                }
                showError('Kamera-Schnittstelle (navigator.mediaDevices) wird von diesem Browser nicht unterstützt.');
                return;
            }
            ...
            try {
                ...
                // Flexible camera constraints with fallback
                try {
                    ...
                    console.warn("Constraints failed, trying generic video: true", constrErr);
                    stream = await navigator.mediaDevices.getUserMedia({
                        video: true,
                        audio: false
                    });
                ...
                // iOS Safari WebKit specific video initialization
                video.muted = true;
                ...
                try {
                    let p = video.play();
                    if (p !== undefined) {
                        p.catch(e => console.warn("video.play() deferred:", e));
                    }
                } catch (playErr) {
                    console.warn("Direct play failed:", playErr);
                }
                ...
                showError(`
                    <strong>Fehler beim Kamerazugriff:</strong> ${err.name || ''} ${err.message || ''}<br><br>
                    Bitte prüfe in den Smartphone-Einstellungen, ob Safari / Chrome auf die Kamera zugreifen darf (Einstellungen ➔ Safari ➔ Kamera: Erlauben).
                `);
            }
        }
        ...
        startCamBtn.addEventListener('click', onToggleCam);
        startCamBtn.addEventListener('touchend', onToggleCam);
        ...
        async function sendFrameLoop() {
            ...
            if (video.videoWidth > 0 && video.videoHeight > 0 && !isSending) {
                let w = 480;
                let h = Math.round(w * (video.videoHeight / video.videoWidth));
                if (h < 240) h = 360;
                if (h > 640) h = 640;
                canvas.width = w;
                canvas.height = h;
                ...
                canvas.toBlob(async (blob) => {
                    if (blob && isStreaming) {
                        try {
                            ...
                            if (res.status === 401) {
                                isStreaming = false;
                                showError('🔑 Ungültige PIN. Bitte lade die Seite neu und gib die richtige PIN ein.');
                                authScreen.style.display = 'block';
                                camScreen.style.display = 'none';
                                isSending = false;
                                return;
                    ...
                    isSending = false;
                    if (isStreaming) {
                        setTimeout(sendFrameLoop, 35); // ~28 FPS
                    }
                }, 'image/jpeg', 0.65);
            } else {
