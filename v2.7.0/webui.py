    ...
    return f"""
        <div class="aether-header-bar-inner">
            <div class="aether-brand-title-group">
                ...
                <span class="aether-studio-version">v2.7.0 NEXT-GEN</span>
...
title = 'Aether Diffusion Studio Pro 2.7.0 • Autonomous Neural AI'
...
""")
                    ...
                    with gr.Tab(label='Describe & Vision Studio', id='describe_tab') as describe_tab:
                        ...
                        with gr.Row():
                            with gr.Column(scale=1):
                                ...
                                vision_status_text = gr.Markdown('*(Bereit. Lade ein Bild hoch und klicke auf "Bild detailliert analysieren")*')
                    ...
                    with gr.Tab(label='🎭 Live Face Swap & Avatar Studio (Beta)', id='faceswap_tab') as faceswap_tab:
                        ...
                        with gr.Row():
                            with gr.Column(scale=1):
                                gr.Markdown("### 📹 1. Deine Webcam (Tracking-Eingang)")
                            ...
                            with gr.Column(scale=1):
                                gr.Markdown("### 🎭 3. Live Avatar Ausgabe (OBS-Stream)")
                                faceswap_output_preview = gr.Image(
                                    label='Echtzeit-Vorschau',
                                    type='numpy',
                                    height=340
                                )
                                faceswap_metrics_text = gr.Markdown("*(Warte auf Webcam-Start...)*")

                        gr.HTML("""
                        <div class="faceswap-obs-box">
                            <h4>🎥 OBS Studio Live-Kamera Einbindung (In 10 Sekunden einsatzbereit)</h4>
                            <p>So bindest du diesen Live-Avatar als virtuelle Kamera oder Stream-Quelle in <strong>OBS Studio</strong> ein:</p>
                            <ol>
                                <li>Öffne <strong>OBS Studio</strong> und klicke im Bereich <em>Quellen (Sources)</em> auf <strong>+ ➔ Browser</strong>.</li>
                                <li>Trage als URL ein: <code style="color: #00ffcc; background: #111; padding: 2px 6px; border-radius: 4px;">http://127.0.0.1:7865/live_avatar</code> (oder für Direkt-Video-Stream: <code style="color: #00ffcc; background: #111; padding: 2px 6px; border-radius: 4px;">http://127.0.0.1:7865/stream/avatar.mjpg</code>).</li>
                                <li>Stelle die Breite auf <code>1024</code> und Höhe auf <code>1024</code> (oder <code>1280x720</code>).</li>
                                <li><strong>Fertig!</strong> Dein animierter Avatar reagiert in Echtzeit auf deine Augen und Lippenbewegungen direkt in deinem Stream oder Discord/Teams-Call!</li>
                            </ol>
                        </div>
                        """)
                        ...
                        faceswap_webcam_input.stream(
                            live_faceswap.process_live_frame,
                            inputs=[faceswap_webcam_input, faceswap_avatar_input, faceswap_mode_radio, faceswap_eye_sens, faceswap_mouth_sens],
...
- **Zielordner:** `{data['target_dir']}`
        ...
        def trigger_vision_analyze(img_input, mask_mode, selected_filters):
            ...
            filtered_prompt = vision_analyzer.build_filtered_prompt(result, selected_filters)
            negative_prompt_val = result.get("negative_prompt", "")
            breakdown_md = vision_analyzer.format_breakdown_markdown(result)
            tags_count = len(result.get("all_tags", []))
            status_msg = f"✅ **Bild lokal analysiert!** ({tags_count} Attribute erkannt • 0 MB VRAM verbleibend)"
        ...
        def trigger_filter_change(cached_data, selected_filters):
            if not cached_data or not cached_data.get("success"):
                return gr.update()
            new_prompt = vision_analyzer.build_filtered_prompt(cached_data, selected_filters)
            return new_prompt
        ...
        vision_analyze_btn.click(
            ...
            inputs=[vision_input_image, vision_mask_mode, vision_attribute_filters],
        ...
        vision_attribute_filters.change(
            ...
            inputs=[vision_state_data, vision_attribute_filters],
            outputs=[vision_result_prompt],
            show_progress=False,
            queue=False
        )
...
# --- OBS Studio Live Streaming Endpoints ---
