...
[![Status: Beta](https://img.shields.io/badge/Status-Beta%20v2.6.3-yellow.svg)](#-community-feedback--bug-reports)
...
> [!NOTE]
> This software is independently developed and maintained by a **single solo developer**. The software is currently in active **Beta phase (v2.6.3 Beta)**.
> 
...
## 🔄 Version Evolution: Fooocus ➔ v2.5 ➔ v2.6 ➔ v2.6.1 ➔ v2.6.2 ➔ v2.6.3

### 🆕 What's New in v2.6.3 (Current Release - Custom ComfyUI Resolution & Negative Prompt Synthesis)
1. **🧪 Official Beta Status Banner & UI Refinements**:
   - Prominent, styled Beta notice banner directly inside the Vision AI Studio tab explaining active development and welcoming community suggestions.
   - Clean, unified action bars (`vision-action-bar`, `vision-res-row`) tidying up button groups into neat functional rows.
2. **❌ Automatic Negative Prompt Synthesis**:
   - The Vision AI analyzes detected style, medium (photorealistic vs. anime/illustration), and visual quality to automatically compose a matching SDXL Negative Prompt.
   - 1-Click button `❌ Als Negativ-Prompt` instantly transfers the generated negative prompt to the main generation pipeline.
3. **📐 Image Resolution Auto-Detection & 1-Click Adoption**:
   - Instantly measures and displays the exact resolution (Width × Height) of any uploaded reference image.
   - Suggests optimal SDXL-compatible dimensions (multiples of 64).
   - 1-Click button `📐 Als Zielauflösung übernehmen` adopts the exact dimensions for rendering.
4. **🎛️ ComfyUI-Style Free Custom Resolution Controls**:
   - Integrated full ComfyUI-style freedom for aspect ratios: width and height sliders (256 - 2048 px, step 64).
   - `🔄 Seitenverhältnis tauschen (Breite ⇄ Höhe)` button for instantaneous portrait/landscape flipping.
   - Natively supported in the SDXL pipeline without fixed ratio lock-in.
5. **💡 In-Depth Upscale Mode Guide**:
   - Built-in visual guide inside the Upscale tab explaining the technical differences between **Fast 2x** (CPU ESRGAN interpolation without new detail synthesis) and **1.5x / 2x** (full SDXL Latent Diffusion denoiser generating realistic micro-textures, hair, and skin details).
...
| Feature | Standard Fooocus (v2.5) | Aether Studio Pro (v2.6) | Aether Studio Pro (v2.6.1) | 🌌 Aether Studio Pro (v2.6.3) |
| :--- | :--- | :--- | :--- | :--- |
...
| **🔬 Vision AI Studio** | Simple describe button | Standard describe | Standard describe | **Precision Vision Studio: Beta Banner, Negative Prompt, Res Detection, Brush Masking, 7 Filters** |
| **📐 Aspect Ratios** | Fixed presets only | Fixed presets only | Fixed presets only | **ComfyUI-Style Custom Pixel Sliders + 1-Click Reference Adoption** |
...
| **🪄 Prompt Creator** | Standard prompt expansion | Prompt Guide Presets | AI Prompt Creator & Enricher | **AI Prompt Magician + Vision Prompt Builder & Negative Generator** |
