...
<body>
    ...
    <div id="ios-http-banner" style="display: none; background: rgba(245, 158, 11, 0.18); border: 1px solid rgba(245, 158, 11, 0.5); border-radius: 14px; padding: 12px; margin-bottom: 14px; max-width: 440px; width: 100%; font-size: 0.85rem; color: #fde68a; line-height: 1.4;">
        ⚠️ <strong>Hinweis für iPhone (Safari):</strong> Apple erlaubt die Kamera nur über <strong>HTTPS</strong>! Klicke am PC auf <strong>„🌐 Öffentlichen Cloud-Tunnel starten“</strong> und scanne den neuen QR-Code (HTTPS: https://....gradio.live).
    </div>
    ...
    <script>
        ...
        // Check if iOS Safari on unencrypted HTTP
            ...
            try {
                if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
                        alert('⚠️ Apple Safari / iPhone Sicherheitssperre:\n\nAuf dem iPhone blockiert Apple die Kamera auf unverschlüsselten Webseiten (http://) im WLAN.\n\n👉 Lösung:\n1. Gehe an deinen PC in Aether Studio Pro.\n2. Klicke auf „🌐 Öffentlichen Cloud-Tunnel starten“.\n3. Scanne den neuen QR-Code (HTTPS: https://....gradio.live).\n\nÜber den sicheren HTTPS-Tunnel erlaubt dein iPhone die Kamera sofort!');
                        return;
                    }
                    throw new Error('Kamera-Schnittstelle (mediaDevices) nicht verfügbar');
                }
                if (stream) {
                ...
                alert('Fehler beim Kamerazugriff: ' + err.message + '\\nBitte erlaube den Kamerazugriff im Handy-Browser (Einstellungen ➔ Safari ➔ Kamera: Erlauben).');
            }
        }
