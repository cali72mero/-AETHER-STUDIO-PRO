...
# 🌌 Aether Diffusion Studio Pro 2.7.2 (Beta)
...
[![Status: Beta](https://img.shields.io/badge/Status-Beta%20v2.7.2-yellow.svg)](#-community-feedback--bug-reports)
[![Release: v2.7.2](https://img.shields.io/badge/Release-v2.7.2-cyan.svg)](v2.7.2/)
...
> [!NOTE]
> This software is independently developed and maintained by a **single solo developer**. The software is currently in active **Beta phase (v2.7.2 Beta)**.
> 
...
## 🔄 Version Evolution: Fooocus ➔ v2.5 ➔ v2.6 ➔ v2.6.3 ➔ v2.7.0 ➔ v2.7.1 ➔ v2.7.2

### 🆕 What's New in v2.7.2 (Current Release - Precision Vision AI: Anime vs. Realism & Target Model Selector)
1. **🎨 Anime vs. Photorealism Selector in Vision AI Studio**:
   - Explicit toggle between:
     - `🔍 Automatisch erkennen (Auto-Detect)`
     - `📸 Realistisches Foto / Fotorealismus (Realism)`
     - `🎨 Anime / Manga / Illustration (Anime)`
   - **Anime Mode**: Strips photographic camera jargon (like `dslr, 35mm, f/1.8 lens, raw photo`) that ruins anime generation, prioritizes Danbooru tags, clean line art, and vibrant 2D cel shading.
   - **Realism Mode**: Strips anime/cartoon tags, prioritizes natural language captioning, detailed skin texture, lighting depth, and camera settings.

2. **🎯 Target Model Dropdown with Syntax Profiling**:
   - Choose which downloaded checkpoint model you plan to use (e.g. `animagineXL40_v4Opt`, `juggernautXL_v8Rundiffusion`, `animaPencilXL`, `jedpointreal`, or `[Automatisch anpassen]`).
   - Automatically adapts prompt syntax and conditioning:
     - **Pony Models**: Injects `score_9, score_8_up, score_7_up, rating_safe` tags and matching negative scores.
     - **Animagine Models**: Injects `masterpiece, best quality, ultra-detailed anime, aesthetic`.
     - **Photorealistic Models**: Injects `raw photo, 8k uhd, dslr, soft cinematic lighting, film grain`.
   - Dynamic 1-millisecond re-calculation when changing model or art type without re-uploading!
...
| Feature | Standard Fooocus (v2.5) | Aether Studio Pro (v2.6) | Aether Studio Pro (v2.6.3) | 🌌 Aether Studio Pro (v2.7.2) |
| :--- | :--- | :--- | :--- | :--- |
...
> [!NOTE]
> Dieses Projekt wird von einem **einzelnen, unabhängigen Entwickler** mit viel Leidenschaft aufgebaut und weiterentwickelt. Das Projekt befindet sich aktuell in der **Beta-Phase (v2.7.2 Beta)**.
> 
...
## 🔄 Versions-Evolution: Was ist neu in v2.7.2?

1. **🎨 Anime vs. Fotorealismus Selektor im Vision AI Studio**:
   - Spezifische Umschaltung zwischen automatischer Erkennung, reinem Fotorealismus oder Anime/Manga.
   - Entfernt unpassende Kamera-Termini bei Anime und injiziert saubere Danbooru-Tags; entfernt Anime-Artefakte bei Fotos.
2. **🎯 Ziel-Modell Profilierung**:
   - Auswahl des Ziel-Checkpoints aus den heruntergeladenen Modellen (z.B. Animagine, Juggernaut, Pony).
   - Passt Syntax und Positiv-/Negativ-Prompts in Echtzeit an die Eigenheiten des gewählten Modells an.
